/* -------------------------------------------------------------------
  ...
  Group {
    ...
    DomainCC_Mag = Region[ {Glue, Ind, Air, AirInf} ] ;
    ...
    DomainS_Mag  = Region[ {} ] ;
    DomainC_Mag  = Region[ {Ind1, Ind2}]; //, Ind} ] ;
  ...
  Function {
    ...
    // Aubert's Value
    //Current[ Region[{Ind}] ] = -135069.; 
    Current[ Region[{Ind1}] ] = -135069.;
    Current[ Region[{Ind2}] ] = -135069.;
    ...
    Time_Fct_U2_Creneau[] = ($Time <= 0.5*TC+Tdelay) ? Time_Fct_Ramp[] : 0;
  ...
  Constraint {
    ...
    Val_U1_ = 1 / (2 * Pi); // 2 Pi since we ...
    Val_U2_ = 1 / (2 * Pi); // 2 Pi since we ...
      
    { Name Voltage_Axi ;
      Case {
        // To use if transient analysis with U(t)
        //{ Region Region[ {Ind} ] ; Value Val_U_; TimeFunction Time_Fct_Creneau[];}
        { Region Region[ {Ind1} ] ; Value Val_U1_; TimeFunction Time_Fct_Creneau[];}
        { Region Region[ {Ind2} ] ; Value Val_U2_; TimeFunction Time_Fct_U2_Creneau[];}
...
*****/
  ...
  PostOperation U_av UsingPost MagDyn_t_av_Axi {
    Print[ R, OnRegion #{Ind1, Ind2},File "test_MagDyn_R.pos" , Format Table, TimeStep {Nbr_Time_Steps:Nbr_Time_Steps}] ;
    Print[ U, OnRegion #{Ind1, Ind2},File "test_MagDyn_U.pos" , Format TimeTable] ;
    Print[ I, OnRegion #{Ind1, Ind2},File "test_MagDyn_I.pos" , Format TimeTable] ;
  }
