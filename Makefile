...
build: clean
	${NPM_BIN}/antora --pull site-dev.yml
...
sync: build
	...
	rsync -avz  --include '*/' --include '*.png' --include '*.jpg' --include '*.jpeg'  --exclude '*' build/site/ es15.siteground.eu:~/public_html/docs.cemosis.fr/
clean:
	rm -rf build
